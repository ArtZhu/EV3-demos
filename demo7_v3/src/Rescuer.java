...
public class Rescuer {
	...
	public static void main(String[] args){
		...
		int[] received = {45, 45, 110, 10, 110, 40, 140, 40};
					//	   2,  2,  1,   4,   4,   4,   5,   4
...
////////////////////////////////////////////////////////////////////////////////
			for(int i=0; i< 8; i++)
				received[i] = receivedList.get(i) + 1;
		}
		...
		for(int i=0; i<8; i+=2){
			Point p;
			if(network)
				p = new Point(received[i], received[i+1]);
			else
				p = Point.actualPoint(received[i], received[i+1]);
	...
	//	boolean moved = false;
		boolean mightblock = false;
		boolean blocked = false;
		for(int i=0; i<order.length; i++){
			Point p = order[i];
			if((p.X == 5 && p.Y == 2) || (p.X == 4 && p.Y == 1)){
				if(!mightblock)
					mightblock = true;
				else
					blocked = true;
	//			moved = true;
			...
			if((p.X == 5 && p.Y == 1) || (p.X == 4 && p.Y == 2) || (p.X == 5 && p.Y == 2) || (p.X == 4 && p.Y == 1)){
				ignore.add(i);
			}
		}
		
		if(blocked){
			order[2] = move(order[2]);
			ignore.remove(ignore.indexOf(2));
		}
...
/*		
		if(moved){
			...
			Iterator<Waypoint> it = pToO.descendingIterator();
			pToO = new LinkedList<Waypoint>();
			while(it.hasNext())
				pToO.add(it.next());
			pToO = simplify(pToO);
	...
	*/	
		for(int i=0; i<4; i++){
			...
			waveFront(order[i]);
			Pose pose1 = poseProvider.getPose();
			LinkedList<Point> pp = pathFrom(Point.actualPoint(pose1.getX(), pose1.getY()));
	...
	//		}
			...
			LinkedList<Point> pathnew = pathFrom(destination[i]);
	...
	private static void exit(){
		InitialGraph[6][3] = 0;
		waveFront(new Point(6, 3));
		Pose p = poseProvider.getPose();
		Iterator<Point> it = pathFrom(Point.actualPoint(p.getX(), p.getY())).descendingIterator();
		LinkedList<Waypoint> pathout = new LinkedList<Waypoint>();
		while(it.hasNext()){
			pathout.add(translate(it.next()));
		}
		pathout = simplify(pathout);
		for(int i=0; i<pathout.size(); i++)
			if(i != 0){
				nav.goTo(pathout.get(i));
				while(nav.isMoving()){}
			}
	...
	private static Point move(Point p){
		...
		Pose pose = poseProvider.getPose();
		Point point = Point.actualPoint(pose.getX(), pose.getY());
		System.out.println("Point: " + point);
		LinkedList<Point> pp = pathFrom(point);
		
		LinkedList<Waypoint> ppw = new LinkedList<Waypoint>();
		
		for(Point p1: pp){
			ppw.add(translate(p1));
			System.out.println("Move preS:");
			System.out.println(p1);
		}
			
		Iterator<Waypoint> it = ppw.descendingIterator();
		ppw = new LinkedList<Waypoint>();
		while(it.hasNext()){
			ppw.add(it.next());
		}
	...
	private static LinkedList<Waypoint> reviseLast(LinkedList<Waypoint> path){
		if(path.size() > 1){
			...
			Pose p = poseProvider.getPose();
			Waypoint prev = new Waypoint(p.getX(), p.getY());
			Waypoint last = path.getLast();
			
			if(last.x - prev.x == 0)
				if(last.y - prev.y > 0)
					path.set(0, new Waypoint(last.x, last.y - grabDist));
				else
					path.set(0, new Waypoint(last.x, last.y + grabDist));
			else if(last.x - prev.x > 0)
				path.set(0, new Waypoint(last.x - grabDist, last.y));
			else
				path.set(0, new Waypoint(last.x + grabDist, last.y));
	...
	private static LinkedList<Point> simplify1(LinkedList<Point> path){
		LinkedList<Point> ans = new LinkedList<Point>();
		ans.add(path.get(0));
		ans.add(path.get(1));
		
		boolean ydir = (path.get(1).X == path.get(1).Y);
	...
	static boolean free(Point p){try{return graph[p.X][p.Y] == 0;}catch(ArrayIndexOutOfBoundsException e){return false;}}
	//Path predicate----------------------------------------------------------------------------------------------------------
