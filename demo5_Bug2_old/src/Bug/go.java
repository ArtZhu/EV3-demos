...
public class go implements Behavior{
	...
	public void action() {
		...
		if(!waveFrontBug2.nav.isMoving()){
			...
			Waypoint next = waveFrontBug2.pathPoint1.get(1);
			

			Point curr1 = Point.actualPoint(waveFrontBug2.mapLength - curr.getY(), curr.getX());
			Point fake1 = Point.actualPoint(waveFrontBug2.mapLength - fake.getY(), fake.getX());
			...
			if(waveFrontBug2.closeTo(fake, curr)){
				Delay.msDelay(500);
				int heading = waveFrontBug2.f1(p.getHeading());
				heading = (heading + 1) % 8 / 2;
				int angle = (waveFrontBug2.dirTo(curr, next) - heading ) * 90;
				if(angle > 180)
					angle = angle - 360;
				else if(angle < -180)
					angle = 360 + angle;
				...
				waveFrontBug2.pilot.rotate(angle);
			...
			//	waveFrontBug2.poseProvider.setPose(new Pose(p.getX(), p.getY(), p.getHeading()+angle));
				...
				waveFrontBug2.pathPoint1.remove(0);
			}
			//start going
			Waypoint wp = waveFrontBug2.pathPoint1.get(0);
			System.out.printf("GO TO: (%f, %f)\n", wp.x, wp.y);
			waveFrontBug2.nav.goTo(wp);
		}
